...
type ShakeHandReqMessage struct {
	Token string `zeroim:"token"`
}

func (m *ShakeHandReqMessage) Encode() ([]byte, error) {
	return json.Marshal(m)
}

func (m *ShakeHandReqMessage) Decode(data []byte) error {
	return json.Unmarshal(data, m)
}

func (m *ShakeHandReqMessage) MsgType() uint16 {
	return ShakeHandReqType
}

type ShakeHandRespMessage struct {
	ErrCode uint16 `zerom:"error_code"`
	ErrMsg  string `zeroim:"error_msg"`
}

func (m *ShakeHandRespMessage) Encode() ([]byte, error) {
	return json.Marshal(m)
}

func (m *ShakeHandRespMessage) Decode(data []byte) error {
	return json.Unmarshal(data, m)
}

func (m *ShakeHandRespMessage) MsgType() uint16 {
	return ShakeHandRespType
}
